...
colnames(wellbeing_tree_data) <- c(
  "employed","Happy","InterestInLife","Satisfied","Community",
  "SocietyBetter","PeopleGood","ManageResponsibilities",
  "TrustingRelationships","Confidence","LifePurpose"
)

# Train/test split
n <- nrow(wellbeing_tree_data)
train_idx <- sample(seq_len(n), size = 0.7 * n)

wb_train <- wellbeing_tree_data[train_idx, ]
wb_test  <- wellbeing_tree_data[-train_idx, ]
...
n2 <- nrow(education_tree_data)
train_idx2 <- sample(seq_len(n2), size = 0.7 * n2)

edu_train <- education_tree_data[train_idx2, ]
edu_test  <- education_tree_data[-train_idx2, ]
...
edu_pred <- predict(education_tree, edu_test, type = "class")
edu_test_acc <- mean(edu_pred == edu_test$employed)
edu_test_acc
```
...
# One common sample containing both the well-being score and the education vars
...
cat("Combined-model sample size:", nrow(combined_data), "person-records\n")
```

```{r}
# Education only (estimated on the combined sample)
combined_edu_only <- glm(
  employed ~ . - WellbeingScore,
  data = combined_data,
  family = binomial
)
...
```{r}
# Does adding well-being improve fit? Likelihood-ratio test, same sample.
anova(combined_edu_only, combined_full, test = "LRT")

# AIC: lower is better. If education-only is not worse, well-being adds nothing.
AIC(combined_edu_only, combined_full)
```

```{r}
# Well-being coefficient before vs after controlling for education
...
wb_compare <- data.frame(
  ...
  Estimate = c(
    as.numeric(wb_alone["Estimate"]),
    as.numeric(wb_combined["Estimate"])
  ),
  SE = c(
    as.numeric(wb_alone["Std. Error"]),
    as.numeric(wb_combined["Std. Error"])
  )
)

wb_compare$Model <- factor(
  wb_compare$Model,
...
ggplot(
  ...
  labs(
    title = "Well-being Coefficient on Employment",
    x = "",
    y = "Logit coefficient (log-odds)"
  )
```
General idea is that well being coeff actually grows when we mix the sample
...
# 1. Well-being distribution (ggplot)
ggsave(
  "wellbeing_distribution.png",
...
# 3. Logit AIC comparison 
ggsave(
  "logit_aic_comparison.png",
...
# 4. Well-being coefficient comparison 
ggsave(
  "wellbeing_coef_comparison.png",
