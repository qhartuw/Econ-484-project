...
\end{equation}
where $\ell(\beta)$ is the log-likelihood and $\lambda$ governs the strength of the $\ell_1$ penalty, which shrinks coefficients toward zero and sets many exactly to zero. The penalty is chosen by cross-validation. we report two standard choices: $\lambda_{\min}$, which minimizes cross-validated deviance, and the more conservative $\lambda_{1se}$, the largest penalty within one standard error of the minimum. The number of variables surviving $\lambda_{1se}$ is a stringent measure of how much robust signal a domain contains.
