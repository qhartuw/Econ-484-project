...
Two broad domains plausibly relate to early-career employment. The first is educational attainment, the canonical measure of human capital. Standard models predict that more schooling, college enrollment, and stronger educational trajectories raise the probability of labor-market participation and employability. The second, less studied domain is subjective well-being. A growing literature in economics and psychology argues that well-being-happiness, life satisfaction, sense of purpose, and social connectedness-constitutes a form of ``psychological capital'' that may support motivation, health, and productivity, and could in turn predict employment. At the same time, the causal arrow may run in either direction, and the two domains are themselves correlated, since education and well-being tend to move together.

This paper addresses two questions. First, do well-being and education each predict current employment among young adults? Second, and more sharply, does well-being add predictive value beyond education, or is any apparent association between well-being and employment simply a reflection of educational differences? The second question matters because comparing two domains in isolation can be misleading: a variable that looks weak on its own may still contribute once correlated factors are controlled, and a variable that looks strong may be redundant.

To answer these questions we take a predictive-modeling approach rather than a causal one. we apply four methods that differ in their assumptions and outputs-logistic regression for interpretable linear effects, classification trees for interpretable nonlinear structure, random forests for flexible prediction and variable importance, and the LASSO for regularized variable selection-and compare what each reveals about the two domains. we then estimate a combined logistic model and use a nested likelihood-ratio test to assess the incremental contribution of well-being. Because the data are a single cross-section, the analysis is explicitly associational: the goal is to characterize which features predict employment, not to identify causal effects.
...
we evaluate the predictive content of the two domains using four methods, chosen so that their strengths and weaknesses are complementary. The outcome in every case is the binary employment indicator.
...
For each domain, the classification tree is used to determine whether the predictors contain enough structure to separate employed from non employed young adults. The analysis begins by constructing a complete case dataset for the domain and dividing it into a training sample containing seventy percent of the observations and a test sample containing the remaining thirty percent. The tree is grown entirely on the training data, which allows the algorithm to search across all predictors and all possible thresholds to identify splits that create more homogeneous groups in terms of employment outcomes. 

Once the tree is grown, it is applied to the held out test sample to obtain an honest measure of predictive accuracy. This procedure allows the model to reveal whether the domain contains any variables that generate stable and interpretable partitions. A tree that produces no splits indicates that none of the predictors meaningfully distinguish employed from non employed respondents. A tree with multiple splits indicates that the domain contains predictive structure that generalizes beyond the training data. The test set accuracy is the relevant performance metric, since it reflects how well the discovered structure predicts employment for new observations. This procedure is applied separately to the well being indicators and the education variables so that the predictive content of the two domains can be compared directly.


\subsection{Random forests}

Random forests are used to assess whether each domain contains nonlinear or interaction based predictive structure that may not be captured by a single classification tree. For each domain, a forest of five hundred trees is grown using bootstrap samples of the complete case dataset. At each split within each tree, a random subset of predictors is considered, which decorrelates the trees and allows the forest to capture a wide range of possible patterns in the data. 

The primary diagnostic from the forest is the out of bag error rate, which is computed by predicting each observation using only the trees for which that observation was not included in the bootstrap sample. This provides an internal estimate of out of sample predictive accuracy without requiring a separate test set. The forest also produces variable importance measures that summarize how much each predictor contributes to reducing classification error across the ensemble. Applying this procedure separately to the well being indicators and the education variables allows the analysis to determine whether either domain contains strong nonlinear predictive signal and which specific variables contribute most to that signal.


\subsection{LASSO}

The LASSO is used to evaluate how much stable predictive information each domain contains once coefficients are penalized toward zero. The method estimates a penalized logistic regression of employment on the predictors in a given domain,
\begin{equation}
\hat\beta = \arg\min_{\beta}\left\{-\ell(\beta) + \lambda \sum_{j} |\beta_j|\right\},
\end{equation}
where $\ell(\beta)$ is the log likelihood and $\lambda$ controls the strength of the penalty that shrinks coefficients toward zero and sets many exactly to zero. For each domain, the predictors are converted into a model matrix and entered into this penalized logistic model.

The penalty parameter is chosen by ten fold cross validation, which evaluates predictive deviance across multiple training and validation splits of the data. Two values of the penalty are reported. The first, $\lambda_{\min}$, is the value that minimizes cross validated deviance and yields the best predictive fit. The second, $\lambda_{1se}$, is the largest penalty within one standard error of the minimum and provides a more conservative measure of robust signal. The number of predictors that remain nonzero under $\lambda_{1se}$ is the key diagnostic, since it reflects how much stable information the domain contains once regularization is applied. A domain with many surviving predictors contains meaningful predictive structure, while a domain with none contains little or no reliable signal. The procedure is applied separately to the well being indicators and the education variables to compare their relative predictive strength.
...
The classification tree is even more emphatic: it produces no splits at all, collapsing to the root node. No well-being item, alone or in combination, separates the employed from the non-employed cleanly enough to warrant a partition. The random forest tells the same story. Its out-of-bag error rate is 48.2\%, which is actually worse than the roughly 43\% error obtained by naively predicting ``employed'' for every respondent. In other words, the ten well-being items contain no usable predictive signal for employment. The variable-importance plot does rank some items above others-satisfaction, managing responsibilities, and trusting relationships appear modestly more important-but in absolute terms the predictive contribution is negligible.

The LASSO confirms this pattern. At $\lambda_{\min}$ a handful of items survive with small coefficients (satisfaction, interest in life, happiness, and managing responsibilities entering positively; community and ``society is getting better'' entering negatively), but at the conservative $\lambda_{1se}$ every well-being variable is shrunk to zero. The model that minimizes cross-validated error is barely distinguishable from one with no predictors. Taken together, the four methods agree that well-being, measured this way, does not predict young-adult employment.
